...
public class BroadBandSnmpUtils {
	...
	public static String retrieveSnmpSetOutputWithGivenIndexOnRdkDevices(Dut device, AutomaticsTapApi tapEnv,
		...
		try {
			...
			if (DeviceModeHandler.isFibreDevice(device)
				...
				// snmpOutput = CommonM.snmpSetOnEstb(tapEnv, device, mibOrOid, dataType, value,
				// tableIndex);
			} else {
	 ...
	 * 
	 * @param tapEnv The {@link AutomaticsTapApi} instance
	 * @param device The device on which snmp command queried
	 * @param oid    The OID or MIB name
	 ...
	 * 
	 * @param device     The device on which snmp command queried
	 * @param snmpOutput Snmp response
	 * @param mib        The OID or MIB name
	 ...
	 * @param tapEnv     AutomaticsTapApi instance
	 * @param device     Dut instance
	 * @param oid        String mib or oid
	 * @param dataType   String datatype
	 * @param value      String value to be set
	 * @param tableIndex String index for mib
	...
	/**
	 * Method to perform reboot using cmd and verify the device is accessible
	 * without verifying the processes are up using snmp
	 * 
	 * * @param tapEnv instance of {@link AutomaticsTapApi}
	 * 
	 * @param device instance of {@link Dut}
...
//		AutomaticsUtils.sleep(AutomaticsConstants.TEN_SECONDS);
	 ...
	 * @param isAccessible True - Do the poll operation to get wait time .
	 * @param hardWaitTime hard wait time after reboot
	 ...
	 * @param tapEnv         {@link AutomaticsTapApi}
	 * @param device         {@link Dut}
	 * @param isFactoryReset True- Perform Factory Reset,False- WiFi Reset
			...
			if (result) {
				...
				if (isFactoryReset) {
					if (!DeviceModeHandler.isRPIDevice(device)) {
						isRebooted = getSystemUpTimeUsingSnmp(tapEnv, device,
						...
						do {
							...
							isRebooted = !CommonMethods.isSTBAccessible(device);
	 ...
	 * @param isSetOper     True - Set Operation, False-Get and verify operation
	...
	/**
	 * Utility Method to set the custom ssid and password for 2.4 and 5 GHz private
	 * wifi
	 * 
	 * @param tapEnv {@link AutomaticsTapApi}
	 * @param device {@link Dut}
