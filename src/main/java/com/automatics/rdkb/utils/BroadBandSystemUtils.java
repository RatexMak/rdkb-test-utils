...
public class BroadBandSystemUtils {
	 ...
	 * @param device         device
	 * @param connectionType connectionType either telnet or ssh
	 * @param ipAddress      Interface address
	 *
	 * @author Arunkumar
	 * @refactor yamini.s
	 */
	...
	/**
	 * Method to Validate telnet connection to ipv4 or ipv6. Collect the ip address
	 * from the ifconfig output, and telnet to ip.
	 *
	 * @param tapEnv        {@link AutomaticsTapApi} Reference
	 * @param device        device
	 * @param interfaceName interfacename for ifconfig.
	 *
	 * @author Arunkumar
	 * @refactor yamini.s
	 */
	 ...
	 * for a given polling time .
	 * 
	 * @param tapEnv         {@link AutomaticsTapApi} Reference
	 * @param device         {@link device} to be validated
	 * @param searchText     String representing the text to be searched in the log
	 *                       file.
	 * @param logFileName    String representing the URL which needs to be present
	 *                       in the searched text.
	 * @param deviceDateTime Timestamp from Device * *
	 * @param pollDuration   polling frequency time
	...
	/**
	 * Method to simulate process crash and verify process restarted with new pid
	 * within 15 mins of crash
	 * 
	 * @param tapEnv      {@link AutomaticsTapApi}
	 * @param device      {@link Dut}
	 * @param processName String containing name of process to be restarted
	 * 
	...
	/**
	 * Helper method to search system ready signal in webpa log file either in ARM
	 * or ATOM console
	 * 
	 * @param tapEnv           {@link AutomaticsTapApi}
	 * @param device           {@link Dut}
	 * @param isAtomSyncDevice is AtomSync device
	 * @return status
	 * 
	 */
	...
	/**
	 * Utility method to verify the webpa log messages on WEBPALog.txt.0 log file
	 * from Arm Console/Atom console for a polling time .
	 * 
	 * @param device            {@link Dut}
	 * @param tapApi            {@link AutomaticsTapApi}
	 * @param webpaNotification String representing the text to be searched in the
	 *                          webpa log file.
	 * @param deviceDateTime    Timestamp from Device * *
	 * @return Boolean representing the result of verification of Log file.
	...
	public static boolean verifyAtomConsoleLog(AutomaticsTapApi tapEnv, Dut device, String searchText,
	 ...
	 * 
	 * @param min integer representing the minimum number.
	 * @param max integer representing the maximum number.
	 * 
	 ...
	 * @param device        {@link Dut}
	 * @param tapApi        {@link AutomaticsTapApi}
	 * @param serviceStatus {@link DNSMASQ_SERVICE_STATUS}
	 * 
	 ...
	 * @throws TestException
	 */
	public static boolean verifyDnsmasqSerivceStatus(Dut device, AutomaticsTapApi tapApi, String serviceStatus)
			throws TestException {
		// stores the verification status
		boolean status = false;
		// stores the command response
		...
		try {
			...
			response = tapApi.executeCommandUsingSsh(device, cmmandForRunningStsDnsMasq);
			LOGGER.info("DNSMASQ service status response : " + response);
			...
			if (CommonMethods.isNotNull(response)) {
				...
				// retrieving the status from systemctl response
				response = CommonMethods.patternFinder(response, patternTogetStatusFromSystmctlResponse);
				LOGGER.info("systemctl dnsmasq service status is: " + response);
				if (CommonMethods.isNotNull(response) && response.trim().equalsIgnoreCase(serviceStatus)) {
					status = true;
					LOGGER.info("systemctl command returned dnsmasq service is " + serviceStatus);
				...
				throw new TestException("NULL response obtained after executing commands" + cmmandForRunningStsDnsMasq
						+ " using remote ssh api in stb");
			}
		} catch (Exception exception) {
			throw new TestException(exception.getMessage());
		...
		try {
			...
			for (int loopCounter = BroadBandTestConstants.CONSTANT_0; loopCounter < backup_files.length; loopCounter++) {
				try {
					...
					result.setStatus(CommonUtils.isFileExists(device, tapEnv, CommonUtils.concatStringUsingStringBuffer(
							BroadBandCommandConstants.MOUNT_NVRAM, backup_files[loopCounter])));
					LOGGER.info("The file " + backup_files[loopCounter] + " is present under /opt");

				} catch (Exception e) {
					result.setStatus(false);
					result.setErrorMessage("The file /opt/" + backup_files[loopCounter] + " is not present under /opt");
				}
			}
		} catch (Exception e) {
			result.setStatus(false);
	...
	public static BroadBandResultObject removeBackupFilesUsedForEncryption(Dut device, AutomaticsTapApi tapEnv) {
		...
		try {
			...
			result.setErrorMessage(
					"Exception occured inside method:: removeBackupFilesUsedForEncryption. " + e.getMessage());
		}
	...
	/**
	 * Utility method to rerieve ip address and domain name for the url from
	 * phishtank site
	 * 
	 * @param CclientDevice {@link Dut} to be validated
	 * 
	 * @param tapEnv        {@value AutomaticsTapApi}
	 * 
	 * @return String array
	 * 
	 * @author Sumathi Gunasekaran
	...
	/**
	 * Method to get Power Level Value via SNMP and WEBPA and cross verify the
	 * values.
	 * 
	 * @param device Dut instance
	 * @param tapEnv AutomaticsTapApi instance
	 * @return BroadbandResultObject with status and Error Message
	 * 
	 * @refactor Athira
	 */
	public static BroadBandResultObject getAndCrossVerifyPowerLevelValueViaSnmpAndWebpa(Dut device,
			AutomaticsTapApi tapEnv) {
		LOGGER.debug("STARTING METHOD: getAndCrossVerifyPowerLevelValueViaSnmpAndWebpa()");
		BroadBandResultObject result = new BroadBandResultObject();
		boolean status = false;
		String webpaStringResponse = null;
		double snmpValueDouble = BroadBandTestConstants.CONSTANT_0;
		double webpaValueDouble = BroadBandTestConstants.CONSTANT_0;
		String response = null;
		String errorMessage = null;
		String powerLevelValue = null;
		try {
			// Since Power Level value keeps changing by time there are posibilities of
			// failure while comparing
			// with value obtained in Step 10. Hence Getting the Power value here before
			// snmpget instantly
			webpaStringResponse = tapEnv.executeWebPaCommand(device,
					BroadBandWebPaConstants.WEBPA_PARAM_DOSCIS_OFDM_POWER_LEVEL
			...
			response = BroadBandSnmpUtils.executeSnmpWalkOnRdkDevices(tapEnv, device,
					BroadBandSnmpMib.POWER_LEVEL.getOid());
			errorMessage = "Unable to retrieve Power Level via WEBPA/SNMP";
			if (CommonMethods.isNotNull(response) && CommonMethods.isNotNull(webpaStringResponse)
				...
				errorMessage = "Power Level value obtained via SNMP/WEBPA Pattern doesnt match!";
				powerLevelValue = CommonMethods.patternMatcher(response,
						BroadBandTestConstants.SNMP_POWER_LEVEL_RESPONSE_REGEX)
								? CommonMethods.patternFinder(response,
										BroadBandTestConstants.SNMP_POWER_LEVEL_RESPONSE_REGEX)
								: CommonMethods.patternFinder(response,
										BroadBandTestConstants.ALTERNATE_SNMP_POWER_LEVEL_RESPONSE_REGEX);
				webpaStringResponse = CommonMethods.patternMatcher(webpaStringResponse,
						BroadBandTestConstants.POWER_LEVEL_RESPONSE_REGEX_FROM_WEBPA)
								? CommonMethods.patternFinder(webpaStringResponse,
										BroadBandTestConstants.POWER_LEVEL_RESPONSE_REGEX_FROM_WEBPA)
								: CommonMethods.patternFinder(webpaStringResponse,
										BroadBandTestConstants.ALTERNATE_POWER_LEVEL_RESPONSE_REGEX_FROM_WEBPA);
				if (CommonMethods.isNotNull(powerLevelValue) && CommonMethods.isNotNull(webpaStringResponse)) {
					errorMessage = "Power Level obtained via SNMP is not matching with the value obtained via webpa";
					LOGGER.info("Power Level Obtained via SNMP : " + powerLevelValue);
					LOGGER.info("Power Level Obtained via WEBPA : " + webpaStringResponse);
					snmpValueDouble = BroadBandCommonUtils.convertStringToDouble(powerLevelValue)
							/ BroadBandTestConstants.CONSTANT_10;
					webpaValueDouble = BroadBandCommonUtils.convertStringToDouble(webpaStringResponse);
					// Expected Variation of Power Level in DB when comparing values between
					// WEBPA/SNMP and vice
					// verse is + or -2 hence threshold values with start and end range is
					// validated. When value
					// variation is Greater than + or - 2 then its a concern.
					status = ((snmpValueDouble >= webpaValueDouble - BroadBandTestConstants.CONSTANT_2
							&& snmpValueDouble <= webpaValueDouble + BroadBandTestConstants.CONSTANT_2)
							|| (webpaValueDouble >= snmpValueDouble - BroadBandTestConstants.CONSTANT_2
									&& webpaValueDouble <= snmpValueDouble + BroadBandTestConstants.CONSTANT_2));
			...
			LOGGER.error(
					"Exception occured while cross validating snr level via snmp with webpa response" + e.getMessage());
		}
		LOGGER.debug("ENDING METHOD: getAndCrossVerifyPowerLevelValueViaSnmpAndWebpa()");
		return result;
	}
	
    /**
     * Method to get SNR Level Value via SNMP and WEBPA and cross verify the values.
     * 
     * @param device
     *            Dut instance
     * @param tapEnv
     ...
     * @refactor Athira
     */
    public static BroadBandResultObject getAndCrossVerifySnrLevelValueViaSnmpAndWebpa(Dut device,
    		AutomaticsTapApi tapEnv) {
	LOGGER.debug("STARTING METHOD: getAndCrossVerifySnrLevelValueViaSnmpAndWebpa()");
	BroadBandResultObject result = new BroadBandResultObject();
	boolean status = false;
	String webpaStringResponse = null;
	double snmpValueDouble = BroadBandTestConstants.CONSTANT_0;
	...
	String response = null;
	String errorMessage = null;
	String snrLevelViaSnmp = null;
	try {
	    // Since Power Level value keeps changing by time there are posibilities of failure while comparing
	    // with value obtained in Step 10. Hence Getting the Power value here before snmpget instantly
	    webpaStringResponse = tapEnv.executeWebPaCommand(device,
		    BroadBandWebPaConstants.WEBPA_PARAM_DOSCIS_OFDM_SNR_LEVEL
			    .replace(BroadBandTestConstants.TR181_NODE_REF, BroadBandTestConstants.STRING_VALUE_ONE));
	    response = BroadBandSnmpUtils.executeSnmpWalkOnRdkDevices(tapEnv, device,
		    BroadBandSnmpMib.SNR_LEVEL.getOid());
	    errorMessage = "Unable to retrieve SNR Level via WEBPA/SNMP";
	    if (CommonMethods.isNotNull(response) && CommonMethods.isNotNull(webpaStringResponse)
		    && !BroadBandCommonUtils.patternSearchFromTargetString(response,
			    BroadBandSnmpConstants.SNMP_ERROR_RESPONSE_NO_OBJECT)
		    && !BroadBandCommonUtils.patternSearchFromTargetString(response,
		    ...
		    && !BroadBandCommonUtils.patternSearchFromTargetString(response,
			    BroadBandTestConstants.SNMPV3_TIMEOUT_ERROR)) {
		errorMessage = "SNR Level value obtained via SNMP/WEBPA Pattern doesnt match!";
		snrLevelViaSnmp = CommonMethods.patternFinder(response,
			BroadBandTestConstants.SNMP_SNR_LEVEL_RESPONSE_REGEX);
		webpaStringResponse = CommonMethods.patternMatcher(webpaStringResponse,
			BroadBandTestConstants.ALTERNATE_SNR_LEVEL_RESPONSE_REGEX_FROM_WEBPA)
				? CommonMethods.patternFinder(webpaStringResponse,
					BroadBandTestConstants.ALTERNATE_SNR_LEVEL_RESPONSE_REGEX_FROM_WEBPA)
				: CommonMethods.patternFinder(webpaStringResponse,
					BroadBandTestConstants.SNR_LEVEL_RESPONSE_REGEX_FROM_WEBPA);
		if (CommonMethods.isNotNull(snrLevelViaSnmp) && CommonMethods.isNotNull(webpaStringResponse)) {
		    errorMessage = "SNR Level obtained via SNMP is not matching with the value obtained via WEBPA";
		    LOGGER.info("SNR Level Obtained via SNMP : " + snrLevelViaSnmp);
		    LOGGER.info("SNR Level Obtained via WEBPA : " + webpaStringResponse);
		    snmpValueDouble = BroadBandCommonUtils.convertStringToDouble(snrLevelViaSnmp)
			    / BroadBandTestConstants.CONSTANT_100;
		    webpaValueDouble = BroadBandCommonUtils.convertStringToDouble(webpaStringResponse);
		    // Expected Variation of SNR Level in DB when comparing values between WEBPA/SNMP and vice verse is
		    // + or -2 hence threshold values with start and end range is validated. When value
		    // variation is Greater than + or - 2 then its a concern.
		    status = ((snmpValueDouble >= webpaValueDouble - BroadBandTestConstants.CONSTANT_2
			    && snmpValueDouble <= webpaValueDouble + BroadBandTestConstants.CONSTANT_2)
			    || (webpaValueDouble >= snmpValueDouble - BroadBandTestConstants.CONSTANT_2
	...
	LOGGER.debug("ENDING METHOD: getAndCrossVerifySnrLevelValueViaSnmpAndWebpa()");
	return result;
    }
