...
public class BroadBandTelemetry2Utils {
	 ...
	 * 
	 * @param device  {@link Instanceof Dut}
	 * @param isDmcli toggle parameter for query method
	 * @return validation result
	 * 
	 ...
	 * @param stepNumber stepnumber to start with
	 * @param testCaseId Testcase ID to update
	 */
	public static void executeTestStepsForTelemetryVersionOne(Dut device, AutomaticsTapApi tapEnv, int stepNumber,
		...
		try {
			...
			/**
			 * VALIDATE IF INFO AND ERROR MESSAGES ARE PRESENT IN
	 ...
	 * 
	 * @param device     {@link Instanceof Dut}
	 * @param uploadTime Telemetry upload time
	...
	public static String validateT2Version(Dut device, AutomaticsTapApi tapEnv) {
		...
		try {
			...
			// } else if
			// (buildName.contains(BroadBandTestConstants.STRING_FEATURE_IN_BUILD_NAME)) {
