...
public class FirmwareDownloadUtils {
    ...
    /**
     * Method to validate the XCONF Configuration retrieved from log file with actual details
     * 
     * @param xconfConfigurations
     *            xconf configuration response as json format
    ...
    /**
     * Utility Method to trigger HTTP CDL using TR-181 Parameters. CDL will be triggered using TR-181 parameter; once
     * the CDL is completed, the device reboots; once the device the image flashing is verified and result is returned.
     * 
     * @param tapEnv
     *            {@link AutomaticsTapApi}
     * @param settop
    ...
    /**
     * Utility method to trigger the TFTP code download via DOCSIS SNMP commands and wait for code download completes
     * and comes up with Requested Image.
     * 
     * @param tapEnv
     *            The {@link AutomaticsTapApi} reference.
     * @param device
     *            The Dut to be flashed.
     * @param buildName
     *            The requested firmware version.
     * @param useImageFrmSystemProperty
     *            Boolean representing the build name to be referred from System Property or the argument passed.
     * 
	...
	for (int index = 0; index < MAX_WAIT_TIME_SNMP_TFTP_CODE_DOWNLOAD; index++) {
		...
		/*
		 * In case of reboot, snmp output will be having error message. So considering this as download
		 * completed.
		 */
		snmpDownloadCompletedStatus = true;
		LOGGER.info(
	...
	if (snmpDownloadCompletedStatus) {
	    /*
	     * if download completed, then wait for device to reboot and comes up with requested image.
	     */
    ...
    /**
     * Utility method to trigger the TFTP code download using DOCS-CABLE-DEVICE-MIB SNMP Commands.
     * 
     * @param device
     *            The device to be validated.
     * @param tapEnv
     *            The {@link AutomaticsTapApi} reference.
     * @param requestedBuildName
     *            The requested build name.
     * @param useImageFrmSystemProperty
     *            Boolean representing the build name to be referred from System Property or the argument passed.
     * @refacor Govardhan
     */
	...
	/**
	 * Commented to extend the method to accommodate the CDL using the Build Name passed as argument
	 */
    ...
    /**
     * Utility method which delete the swupdate.conf in nvram directory to avoid further downloads.
     * 
     * @param tapEnv
     *            The Automaticstap api instance
     * @param device
     *            The Dut instance
     */
    ...
    /**
     * Utility method to trigger the XCONF download via WebPA or Reboot and verify whether code download is started from
     * xconf log message.
     * 
     * 
     * @param tapEnv
     ...
     *            The firmware version to be flashed or triggered.
     * @param isWebPaTrigger
     *            True if XCONF trigger is via WebPA, otherwise it will reboot the device for triggering the XCONF.
     * @return True if XCONF trigger request got accepted. This is based on xconf log messages.
     * @refactor Govardhan
     */
	...
	if (isCdlTriggered) {
	    /*
	     * Some of devices taking more time to initiate the Device initiated firmware download.
	     */
    ...
    /**
     * Utility method to verify whether code download completed during reboot window using log message.
     * 
     * @param response
     * @return True if required logs are available in command response.
     * @refactor Govardhan
    ...
    /**
     * Helper method to validate the download progress using logs. It takes care of device reboot during maintenance
     * window and if download takes more than expected time. This will reduce the false alarm for download failure.
     * 
     * @param tapEnv
     *            {@link AutomaticsTapApi}
     * @param device
     ...
     *            The command to get download status log.
	...
	/*
	 * Different models having different download time, so query the code download file in TWO minutes interval and
	 * check whether required logs are there and exit the loop if present. This will avoid hard wait and failing the
	 * test after hard wait. This increase the efficiency and reduce the false failure.
	 */
	...
	for (int count = 1; count <= 20; count++) {
	    ...
	    /*
	     * Even after 10 minutes device is not showing download status logs, check whether device rebooted during
	     * reboot window and comes up with latest image. If current running image name and requested image names are
	     * same. Please update the status to true. This logic will be changed once configurable maintenance window
	     * feature is implemented in RDKB devices.
	     */
	...
	if (!status && !actualCdlUpgradeFailure) {
	    /*
	     * If status is false even after 10 iteration and there is no failure, try with reboot and then check
	     * version name. There are situations where some part of logs missing which causing the failure. Since QT is
	     * primary entry point for triggering other test cases, so rebooting the device and checking whether device
	     * upgraded with requested image.
	     * 
	     * 
	     */
    ...
    /**
     * Method to trigger Cdl by http protocol using Tr181 param and Tftp protocol, in case the http protocol fails.
     * 
     * 
     * @param tapEnv
     *            {@link AutomaticsTapApi}
     ...
     *            {@link Dut}
     * @param cdlImageWithoutBinExtension
     *            The CDL Image name Without ".bin" Extension
    ...
    /**
     * Helper method to trigger the CDL on the device with the Latest Available Image using HTTP TR-181 Method.
     * 
     * @param tapEnv
     *            {@link AutomaticsTapApi}
     * @param device
	...
	String errorMessage = null;
	boolean result = false;
	// String cdlImageWithoutBinExtension = getLatestAvailableImageForBroadBandDevices(device, tapEnv,
