...
public class BroadBandPostConditionUtils {
     ...
     *            String to hold post condition number.
     */
    public static void executePostConditionToDisablePublicWifi(Dut device, AutomaticsTapApi tapEnv,
	    int postConStepNumber) {
	boolean status = false;
	String errorMessage = null;
	try {
	    LOGGER.info("#######################################################################################");
	    LOGGER.info("POST-CONDITION " + postConStepNumber
		    + " : DESCRIPTION : VERIFY THE PUBLICWIFI STATUS IS DISABLED");
     ...
     * @param tapEnv
     *            instance of {@link AutomaticsTapApi}
     * @param defaultChannelValuesMap
     *            Its used to get the key and pair value for default channel values
    ...
    /**
     * Method to execute Post-condition steps for Band steering Disabling Bandsteering and Enabling Mesh via WebPa
     * 
     * @param tapEnv
     *            instance of {@link AutomaticsTapApi}
     * @param device
	...
	try {
	    /**
	     * POST-CONDITION :DISCONNECTING THE WI-FI CLIENT CONNECTED WITH 5GHz SSID USING WEBPA
	     */
	    status = false;
	    errorMessage = null;
	    LOGGER.info("#######################################################################################");
     ...
     * @refactor Govardhan
     */
    public static void postConditionCloseBrowser(boolean isBrowserOpen, Integer stepNumber) {
    ...
    /**
     * POST condition to reboot the device and wait for ip accusation
     * 
     * @param device
     *            {@link Dut}
     * @param tapEnv
     ...
     * @refactor Rakesh C N
     */
    public static void postConditionToRebootAndWaitForIpAccusition(Dut device, AutomaticsTapApi tapEnv,
	    int postConStepNumber) {
	String errorMessage = null;
	boolean status = false;
	/**
	 * POST-CONDITION : REBOOT THE DEVICE AND WAIT FOR IP ACCUSITION
	 */
	LOGGER.info("#######################################################################################");
	LOGGER.info("POST-CONDITION " + postConStepNumber
		+ " : DESCRIPTION : REBOOT THE DEVICE AND WAIT FOR IP ACCUSITION");
	LOGGER.info("POST-CONDITION " + postConStepNumber + " : ACTION : EXECUTE COMMAND : /sbin/reboot");
	LOGGER.info("POST-CONDITION " + postConStepNumber + " : EXPECTED : DEVICE REBOOT SHOULD BE SUCCESSFUL");
	LOGGER.info("#######################################################################################");
	...
	if (status) {
	    LOGGER.info("POST-CONDITION " + postConStepNumber
		    + " : ACTUAL : DEVICE REBOOT AND IP ACCUSITION IS SUCCESSFUL");
	} else {
	    LOGGER.error("POST-CONDITION " + postConStepNumber + " : ACTUAL : " + errorMessage);
     ...
     * @refactor Athira
     */
    public static boolean executePostConditionToSetTheDefaultDchpIpv4Values(Dut device, AutomaticsTapApi tapEnv,
    ...
    /**
     * Post-Condition method to revert DHCPV4 Default configurations
     * 
     * @param device
     *            {@link Dut}
     * @param tapApi
     ...
     * 
     */
    public static void executePostConditionToRevertDhcpv4ConfigurationToDefaultValues(Dut device,
	    AutomaticsTapApi tapEnv, Map<String, String> defaultDhcpWebpaOutput, String postConditionNumber) {
	boolean leaseTimeStatus = false;
	boolean beginningValueStatus = false;
	boolean endingValueStatus = false;
	boolean status = false;
	LOGGER.info("POST-CONDITION " + postConditionNumber
		+ ": DESCRIPTION : Revert the DHCP Lease Time and setting the DHCPV4 Beginning and Ending Address to default values.");
	LOGGER.info("POST-CONDITION " + postConditionNumber
		+ ": EXPECTED : Reverting DHCP Lease time,DHCPV4 Beginning and Ending Address to default value should be successful.");
	try {

	    leaseTimeStatus = BroadBandWebPaUtils.setVerifyWebPAInPolledDuration(device, tapEnv,
		    BroadBandWebPaConstants.WEBPA_PARAMETER_FOR_DHCP_LEASETIME, BroadBandTestConstants.CONSTANT_1,
		    defaultDhcpWebpaOutput.get(BroadBandWebPaConstants.WEBPA_PARAMETER_FOR_DHCP_LEASETIME),
		    BroadBandTestConstants.THREE_MINUTE_IN_MILLIS, BroadBandTestConstants.THIRTY_SECOND_IN_MILLIS);

	    beginningValueStatus = BroadBandWebPaUtils.setVerifyWebPAInPolledDuration(device, tapEnv,
		    BroadBandWebPaConstants.WEBPA_PARAMETER_FOR_DHCP_MINADDRESS, BroadBandTestConstants.CONSTANT_0,
		    defaultDhcpWebpaOutput.get(BroadBandWebPaConstants.WEBPA_PARAMETER_FOR_DHCP_MINADDRESS),
		    BroadBandTestConstants.THREE_MINUTE_IN_MILLIS, BroadBandTestConstants.THIRTY_SECOND_IN_MILLIS);
	    endingValueStatus = BroadBandWebPaUtils.setVerifyWebPAInPolledDuration(device, tapEnv,
		    BroadBandWebPaConstants.WEBPA_PARAMETER_FOR_DHCP_MAXADDRESS, BroadBandTestConstants.CONSTANT_0,
		    defaultDhcpWebpaOutput.get(BroadBandWebPaConstants.WEBPA_PARAMETER_FOR_DHCP_MAXADDRESS),
		    BroadBandTestConstants.THREE_MINUTE_IN_MILLIS, BroadBandTestConstants.THIRTY_SECOND_IN_MILLIS);
	    status = leaseTimeStatus && beginningValueStatus && endingValueStatus;
	    if (status) {
		LOGGER.info("POST-CONDITION " + postConditionNumber
			+ ": ACTUAL : Reverting back DHCPV4 to default values executed successfully.");
	    } else {
		LOGGER.info("POST-CONDITION " + postConditionNumber
			+ ": Reverting Back to default values was not successful.");
	    }
	} catch (Exception e) {
	    LOGGER.info(e.getMessage());
	}
