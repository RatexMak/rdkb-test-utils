...
public class BroadBandCommonPage extends BroadBandBasePage {
	...
	/**
	 * Method to get the port mode enable status using "aria-checked" attribute of
	 * Enable and Disable buttons.
	 * 
	 * @return boolean true if portfowarding is enabled /false if portfowarding is
	 *         in disabled state
	 * 
	 * @refactor Rakesh C N
	 **/
	...
	/**
	 * Method to get the https port mode enable status using "aria-checked"
	 * attribute of Enable and Disable buttons.
	 * 
	 ...
	 * @param setToReservedIp   boolean to switch between DHCP and Reserved IP
	 * @param clientMacAddress  Wi-Fi/Ethernet Client MAC Address
	 * @param reservedIpAddress IPv4 address in DHCP Range
	...
	/**
	 * Utility method to modify custom Firewall setting for IPV4 or IPV6 from
	 * Firewall Page
	 * 
	 * @param tapEnv
	 * @param settop
	 ...
	 * @refactor Athira
	 */
	public static boolean configureCustomFirewallSetting(AutomaticsTapApi tapEnv, Dut device, WebDriver driver,
			String elementId, String securityLevel, String ipInterface, String modifyType) {

		LOGGER.debug("STARTING METHOD : configureCustomFirewallSetting()");
		...
		try {
			...
			if (BroadBandCommonUtils.compareValues(BroadBandTestConstants.CONSTANT_TXT_COMPARISON,
					BroadBandWebGuiTestConstant.XPATH_FOR_BLOCK_ICMP, modifyType)) {
				LOGGER.info("Selecting Block ICMP Firewall setting by clicking the security level!");
				WebElement element = (new WebDriverWait(driver, 30)).until(ExpectedConditions
						.elementToBeClickable(By.xpath(BroadBandWebGuiTestConstant.XPATH_FOR_BLOCK_ICMP)));
				element.click();
				LOGGER.info("clicking the 'Save Settings' button!");
				driver.findElement(By.id(BroadBandWebGuiElements.ELEMENT_ID_SUBMIT_FIREWALL_BUTTON)).click();
				LOGGER.info("Waiting for 20 seconds for the changes to reflect!");
				tapEnv.waitTill(BroadBandTestConstants.TWENTY_SECOND_IN_MILLIS);
				status = BroadBandCommonUtils.compareValues(BroadBandTestConstants.CONSTANT_TXT_COMPARISON,
						securityLevel, BroadBandCommonPage.getFirewallSecurityLevel(driver, device, ipInterface));
			} else {
				WebDriverWait wait = new WebDriverWait(driver, 30);
				LOGGER.info("Selecting Disable entire Firewall setting by clicking the security level!");
				WebElement element = (new WebDriverWait(driver, 20)).until(ExpectedConditions
						.elementToBeClickable(By.xpath(BroadBandWebGuiElements.XPATH_DISABLE_ENTIRE_FIREWALL)));
				element.click();
				wait = new WebDriverWait(driver, 100);
				LOGGER.info("Comparing Pop Up value for diable entire firewall");
				status = CommonMethods.patternMatcher(
						driver.findElement(By.xpath(BroadBandWebGuiElements.XPATH_FOR_ETHWAN_POPUP_MESSAGE)).getText(),
						BroadBandTestConstants.CUSTOM_SECURITY_POP_UP_MESSAGE);
				LOGGER.info(
						driver.findElement(By.xpath(BroadBandWebGuiElements.XPATH_FOR_ETHWAN_POPUP_MESSAGE)).getText());
				driver.findElement(By.xpath(BroadBandWebGuiElements.XPATH_FOR_CANCEL_IN_ETHWAN_POPUP_MESSAGE)).click();
			}
		...
		LOGGER.debug("ENDING METHOD : configureCustomFirewallSetting()");
		return status;
	}

	/**
	 * Method to get ofdm values from network page
	 * 
	 * @param driver        WebDriver
	 * @param channelParams Map of channel id's
	 * @return List of BroadbandDownStreamGui objects
	 * 
	 * @refactor Athira
	 */
	public static List<BroadbandDownStreamGui> getOfdmValuesFromNetworkPage(WebDriver driver,
			Map<String, String> channelParams, boolean isLanPage) {
		LOGGER.debug("STARTING METHOD getOfdmValuesFromNetworkPage()");
		List<BroadbandDownStreamGui> listOfOfdmValues = new ArrayList<BroadbandDownStreamGui>();
		BroadbandDownStreamGui broadbandDownStreamGui = new BroadbandDownStreamGui();
		String channelID = null;
		String snrLevel = null;
		String powerLevel = null;
		String xpathCommon = isLanPage
				? BroadBandWebGuiElements.XPATH_DOCSIS_DOWNSTREAM_TABLE.replace(BroadBandTestConstants.STRING_REPLACE,
						BroadBandTestConstants.STRING_5)
				: BroadBandWebGuiElements.XPATH_DOCSIS_DOWNSTREAM_TABLE.replace(BroadBandTestConstants.STRING_REPLACE,
						BroadBandTestConstants.STRING_VALUE_12);
		int maxChannel = Integer.valueOf(channelParams.get(BroadBandWebPaConstants.WEBPA_PARAM_DOSCIS_OFDM_CHANNELID
				.replace(BroadBandTestConstants.TR181_NODE_REF, String.valueOf(channelParams.size()))));
		LOGGER.info("Last ChannelID:" + maxChannel);
		Map<String, String> channelRow = new HashMap<String, String>();
		String pointer = null;
		try {
			for (int count = 1; count <= maxChannel; count++) {
				try {
					channelID = driver.findElement(By.xpath(xpathCommon
							.replace(BroadBandTestConstants.PLACE_HOLDER_FOR_ROW_NUMBER,
									BroadBandTestConstants.STRING_VALUE_ONE)
							.replace(BroadBandTestConstants.PLACE_HOLDER_FOR_DIV_1_NUMBER, String.valueOf(count))))
							.getText();
					LOGGER.info("Channel id: " + channelID + " Row: " + count);
					channelRow.put(channelID, String.valueOf(count));
				} catch (Exception e) {
			...
			for (int count = 1; count < channelParams.size() + 1; count++) {
				...
				pointer = channelParams.get(BroadBandWebPaConstants.WEBPA_PARAM_DOSCIS_OFDM_CHANNELID
						.replace(BroadBandTestConstants.TR181_NODE_REF, String.valueOf(count)));

				channelID = driver
						.findElement(By.xpath(xpathCommon
								.replace(BroadBandTestConstants.PLACE_HOLDER_FOR_ROW_NUMBER,
										BroadBandTestConstants.STRING_VALUE_ONE)
								.replace(BroadBandTestConstants.PLACE_HOLDER_FOR_DIV_1_NUMBER,
										channelRow.get(pointer))))
						.getText();
				...
				powerLevel = driver.findElement(By.xpath(xpathCommon
						.replace(BroadBandTestConstants.PLACE_HOLDER_FOR_ROW_NUMBER, BroadBandTestConstants.STRING_5)
						.replace(BroadBandTestConstants.PLACE_HOLDER_FOR_DIV_1_NUMBER, channelRow.get(pointer))))
						.getText();
				LOGGER.info("Channelid: " + channelID + " SNR: " + snrLevel + " Power level: " + powerLevel);
				if (CommonMethods.isNotNull(channelID) && CommonMethods.isNotNull(snrLevel)
						&& CommonMethods.isNotNull(powerLevel)) {
					broadbandDownStreamGui.setIndex(channelID);
					broadbandDownStreamGui.setSnr(snrLevel);
					broadbandDownStreamGui.setPowerLevel(powerLevel);
					listOfOfdmValues.add(broadbandDownStreamGui);
				}
			}
		} catch (Exception e) {
			LOGGER.error("Exception caught while retrieving values from GUI");
		}
		LOGGER.debug("ENDING METHOD getOfdmValuesFromNetworkPage()");
		return listOfOfdmValues;
	}
