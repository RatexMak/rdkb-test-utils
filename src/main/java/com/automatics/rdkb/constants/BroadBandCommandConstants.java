...
public class BroadBandCommandConstants extends RDKBTestConstants {
	...
	/**
	 * Command to get the configuration download details from
	...
	/**
	 * Command to check the difference between /nvram/syscfg.db and
	 * /opt/secure/data/syscfg.db files
	 */
	...
	/**
	 * File path for Encrypted ManagementServer ConnectionRequestPassword stored in
	 * nvram
	 */
	...
	public enum ATOMSIDE_FILES {
		ATF_BIN(""), UTF_BIN(""), OTP_BIN(""), BD3GLD_BIN(""), BD2GLD_BIN(""), BD2CUSGLD_BIN(""), FAKE_BIN(""),
		FAKEBOARD("");
	...
	public enum ATOM_REMOVAL_FILES {
		EZIPUPDATE("/usr/bin/ez-ipupdate"), DCA_UTILITY("/rdklogger/dca_utility.sh"),
		USBHIDDUMP("/usr/bin/usbhid-dump"), DCASPLUNKUPLOAD("/rdklogger/dcaSplunkUpload.sh"),
		DCMSCRIPT("/rdklogger/DCMscript.sh"), ESTSECUREWRAPPER("/usr/bin/testsecurewrapper"), LTIME("/usr/bin/LTime"),
		STARTTUNNEL("/rdklogger/startTunnel.sh"), RFC_REFRESH("/rdklogger/rfc_refresh.sh"),
		GETPARTNERID("/rdklogger/getpartnerid.sh"), GETACCOUNTID("/rdklogger/getaccountid.sh"),
		...
		APPLY_PARTNER_CUSTOMIZATION("/rdklogger/apply_partner_customization.sh"), XHSSCRIPT("/rdklogger/xhsScript.sh"),
		GETIPV6_CONTAINER("/rdklogger/getipv6_container.sh"), RFC("/rdklogger/rfc.sh"),
		GETIP_FILE("/rdklogger/getip_file.sh"), LOG_TIMESTAMP("/rdklogger/log_timestamp.sh"),
		FILEUPLOADRANDOM("rdklogger/fileUploadRandom.sh"), WAN_SSH("/rdklogger/wan_ssh.sh"),
		EXPECT_CREATE_USERSPACE("/usr/bin/expect_create_userspace"), EXPECT_CREATE_NAT("/usr/bin/expect_create_nat"),
		EXPECT_CREATE("/usr/bin/expect_create"), CONNTRACK_FILTER("/usr/bin/conntrack_filter"),
		CONNTRACK_MASTER("/usr/bin/conntrack_master"), EXPECT_GET("/usr/bin/expect_get"),
		CONNTRACK_GET("/usr/bin/conntrack_get"), EXPECT_DELETE("/usr/bin/expect_delete"),
		CONNTRACK_GRP_CREATE("/usr/bin/conntrack_grp_create"), CONNTRACK_CREATE_NAT("/usr/bin/conntrack_create_nat"),
		CONNTRACK_UPDATE("/usr/bin/conntrack_update"), CONNTRACK_CREATE("/usr/bin/conntrack_create"),
		CONNTRACK_DUMP_FILTER("/usr/bin/conntrack_dump_filter"), CTEXP_EVENTS("/usr/bin/ctexp_events"),
		CONNTRACK_DELETE("/usr/bin/conntrack_delete"), EXPECT_EVENTS("/usr/bin/expect_events"),
		CONNTRACK_EVENTS("/usr/bin/conntrack_events"), CONNTRACK_DUMP("/usr/bin/conntrack_dump"),
		EXPECT_DUMP("/usr/bin/expect_dump"), EXPECT_FLUSH("/usr/bin/expect_flush"),
		CONNTRACK_FLUSH("/usr/bin/conntrack_flush"), SERVICE_DHCP("/usr/bin/service_dhcp"),
		SERVICE_MULTINET_EXEC("/usr/bin/service_multinet_exec"),
		APPLY_SYSTEM_DEFAULTS("/usr/bin/apply_system_defaults"), SERVICE_IPV6("/usr/bin/service_ipv6"),
		SERVICE_WAN("/usr/bin/service_wan"), SERVICE_UDHCPC("/usr/bin/service_udhcpc"),
		DHCP_PROXY("/usr/bin/dhcp_proxy"), SERVICE_ROUTED("/usr/bin/service_routed"),
		NFQ_HANDLER("/usr/bin/nfq_handler"), SYSEVENTD_PROXY("/usr/bin/syseventd_proxy"), NEWHOST("/usr/bin/newhost"),
		PMON("/usr/bin/pmon"), EXECUTE_DIR("/usr/bin/execute_dir"), UTCMD("/usr/bin/utcmd"),
		INIT_01("/usr/bin/01_init"), WAN_IPV6_03("/usr/bin/03_wan_ipv6"), BYOI_04("/usr/bin/04_byoi"),
		BYOI_GAP_04("/usr/bin/04_byoi_gap"), IPV6_02("/usr/bin/02_ipv6"), QOS_15("/usr/bin/15_qos"),
		BOOTSTRAP_DNS_10("/usr/bin/10_bootstrap_dns"), XHS_15("/usr/bin/15_xhs"),
		DNSFORWARDER_15("/usr/bin/15_dnsforwarder"), MCASTSNOOPER_10("/usr/bin/10_mcastsnooper"),
		NTPCLIENT_10("/usr/bin/10_ntpclient"), FACTORYDEFAULT_20("/usr/bin/20_factorydefault"),
		CISCOCONNECT_20("/usr/bin/20_ciscoconnect"), GWRESET_20("/usr/bin/20_gwreset"),
		SWITCHPMON_15("/usr/bin/15_switchpmon"), MANAGED_02("/usr/bin/02_managed"), SYSTEM_01("/usr/bin/01_system"),
		FPM_10("/usr/bin/10_fpm"), WLAN_02("/usr/bin/02_wlan"), LAN_02("/usr/bin/02_lan"),
		FTP_SERVER_15("/usr/bin/15_ftp_server"), IGD("/usr/bin/IGD"), IGD_15("/usr/bin/15_igd"),
		FIREWALL("/usr/bin/firewall"), GENFWLOG("/usr/bin/GenFWLog"), FIREWALL_10("/usr/bin/10_firewall"),
		IPV4_02("/usr/bin/02_ipv4"), HOTSPOT_15("/usr/bin/15_hotspot"), DHCPV6_SERVER_15("/usr/bin/15_dhcpv6_server"),
		WAN_02("/usr/bin/02_wan"), LANHANDLER_02("/usr/bin/02_lanHandler"),
		DHCPV6_CLIENT_15("/usr/bin/15_dhcpv6_client"), MCASTPROXY_10("/usr/bin/10_mcastproxy"),
		CCSPHS_15("/usr/bin/15_ccsphs"), MLDPROXY_10("/usr/bin/10_mldproxy"), DDNSCLIENT_15("/usr/bin/15_ddnsclient"),
		POTD_26("/usr/bin/26_potd"), MISC_15("/usr/bin/15_misc"), XDNS_09("/usr/bin/09_xdns"),
		NTPD_10("/usr/bin/10_ntpd"), PARODUS_02("/usr/bin/02_parodus"), FORWARDING_02("/usr/bin/02_forwarding"),
		BRIDGE_02("/usr/bin/02_bridge"), COSA_33("/usr/bin/33_cosa");
	...
	public enum ARM_REMOVAL_FILES {
		...
		DCA_UTILITY("/rdklogger/dca_utility.sh"), DCASPLUNKUPLOAD("/rdklogger/dcaSplunkUpload.sh"),
		DCMCRONRESCHEDULE("/rdklogger/DCMCronreschedule.sh"), DCMSCRIPT("/rdklogger/DCMscript.sh"),
		GETACCOUNTID("/rdklogger/getaccountid.sh"), GETPARTNERID("/rdklogger/getpartnerid.sh"),
		INITZRAM("/rdklogger/init-zram.sh"), RFC_REFRESH("/rdklogger/rfc_refresh.sh"),
		STARTTUNNEL("/rdklogger/startTunnel.sh"), TELEMETRYEVENTLISTENER("/rdklogger/telemetryEventListener.sh"),
		UTILS("/rdklogger/utils.sh"), WAN_SSH("/rdklogger/wan_ssh.sh");
